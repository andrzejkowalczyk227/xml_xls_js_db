...
<xsl:param name="pWlasnosc"/>
<xsl:param name="pWartosc"/>
  ...
  <body>
    ...
    <div class="sprzetyTable">
		<div class="sprzetyBody">
			...
			<xsl:apply-templates select="/Wypozyczalnia/Magazyn//*[name()=$pWlasnosc and text()=$pWartosc]/ancestor::Sprzet"/>
		</div>
	</div>
		...
		<div class="przedmiotyTable" id="przedmiot{$pozycja}">
			<div class="przedmiotyBody">
				<!-- podejscie proceduralno/iteracyjne
					 ze wzgledu na swobodne definicje typow przedmiotow,
		...
		<xsl:if test="count(ancestor::Serwis)=0">
			<xsl:choose>
				<xsl:when test="name()='Zdjecie'">
					<div class="przedmiotyCell przedmiotyCellImg"><img src="{concat($hostname,text())}"/></div>
				</xsl:when>
				<xsl:otherwise>
